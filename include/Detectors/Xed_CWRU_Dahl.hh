...
class DetectorExample_XED : public VDetector {
 public:
  ...
  virtual void Initialization() override {
    ...
    // S2botTotRatio = 0.5; //bottom-to-total, typically only used for position
    // recon (1-this)
    ...
    // PMTs) In 2-phase, TopDrift=liquid/gas border. In gas detector it's GATE,
    // not anode!
    ...
    gate = 8.0;  // mm. This is where the E-field changes (higher)
                 // in gas detectors, the gate is still the gate, but it's where
                 // S2 starts
  ...
  virtual double FitEF(double xPos_mm, double yPos_mm,
    ...
    if (switcher == 0)
      ...
      electricField = 4500;
    if (electricField < FIELD_MIN) electricField = FIELD_MIN;
